...
while i<len(dati):
    ...
    text='{"config": { "service_url": "http://localhost:8080/rasdaman/ows", "tmp_directory": "/tmp/", "crs_resolver": "http://localhost:8080/def/",'\
        ...
        ' "input": { "coverage_id": "{0}", "paths": [ "{1}{0}_20*" ] }, "recipe": { "name": "time_series_irregular", '\
        '"options": {"wms_import": false, "time_parameter": { "filename": { "regex": "(.*)_(.*)", "group": "2" },'\
        ' "datetime_format": "YYYYMMDD"}, "time_crs": "http://localhost:8080/def/crs/OGC/0/AnsiDate", '\
        '"tiling": "ALIGNED [0:1023, 0:1023] TILE SIZE 4194304" } } }'.format(dati[i], new_percorso)
    print(text)
